...
object JsonToRawDataClass {
    ...
    fun testJson() {
        ...
        val categories: Array<CategoryClass> = gson.fromJson(gsonCategoriesArray.toString(), arrayTutorialType)
        for(i in categories){
    ...
    fun moneyInteractionJson(gsonString: String?) : MoneyInteractionClass? {
        return if(gsonString != null) {
            val moneyInteraction: MoneyInteractionClass = Gson().fromJson(gsonString, MoneyInteractionClass::class.java)
    ...
    fun toMoneyInteractionJson(moneyInteraction: MoneyInteractionClass?) : String? {
